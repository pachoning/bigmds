test_that("Partitions for Divide and Conquer returns a valid partition dataset", {
  partition <- get_partitions_for_divide_conquer(n = 1000, l = 100, num_stitching_points = 5, k = 10)
  p <- max(partition)
  expect_equal(p, 11)
})
...
test_that("Divide and Conquer MDS returns a valid MDS configuration", {
  ...
  cmds <- divide_conquer_mds(x = x, l = 100, num_stitching_points = 8, k = 4, dist_fn = stats::dist)
